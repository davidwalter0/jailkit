/* POSIX.2 wordexp implementation.
   ...
   Changes to make this compile on FreeBSD (C) 2004 Olivier Sessink
...
static int parse_dollars(char **word, size_t * word_length, size_t * max_length, const char *words,
						 size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
						 const char *ifs_white, int quoted);
static int parse_backtick(char **word, size_t * word_length, size_t * max_length, const char *words,
						  size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
						  const char *ifs_white);
static int parse_dquote(char **word, size_t * word_length, size_t * max_length, const char *words,
						size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
						const char *ifs_white);
...
static inline char *w_addchar(char *buffer, size_t * actlen, size_t * maxlen, char ch)
...
static char *w_addstr(char *buffer, size_t * actlen, size_t * maxlen, const char *str)
	 /* (lengths exclude trailing zero) */
{
	/* Add a string to the buffer, allocating room for it if needed.
	 */
...
static int
 parse_backslash(char **word, size_t * word_length, size_t * max_length, const char *words,
				 size_t * offset)
{
...
static int
 parse_qtd_backslash(char **word, size_t * word_length, size_t * max_length, const char *words,
					 size_t * offset)
{
...
static int
 parse_tilde(char **word, size_t * word_length, size_t * max_length, const char *words,
			 size_t * offset, size_t wordc)
{
	...
	if (i == 1 + *offset) {
		...
		uid = getuid();
		/*
		while ((result = getpwuid_r(uid, &pwd, buffer, buflen, &tpwd)) != 0 && errno == ERANGE) {
			buflen += 1000;
			buffer = alloca(buflen);
		}*/
		tpwd = getpwuid(uid);

		if (tpwd != NULL && tpwd->pw_dir != NULL) {
			*word = w_addstr(*word, word_length, max_length, tpwd->pw_dir);
			if (*word == NULL)
				return WRDE_NOSPACE;
		} else {
			*word = w_addchar(*word, word_length, max_length, '~');
		...
		char *user = strndup(&words[1 + *offset], i - *offset-1);
		struct passwd *tpwd;
/*		printf("looking for user '%s'\n",user);*/
		tpwd = getpwnam(user);

		if (tpwd != NULL && tpwd->pw_dir)
			*word = w_addstr(*word, word_length, max_length, tpwd->pw_dir);
		else {
			/* (invalid login name) */
			*word = w_addchar(*word, word_length, max_length, '~');
...
static int
 do_parse_glob(const char *glob_word, char **word, size_t * word_length, size_t * max_length,
...
static int
 parse_glob(char **word, size_t * word_length, size_t * max_length, const char *words,
			size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
			const char *ifs_white)
{
	...
	for (; words[*offset] != '\0'; ++*offset) {
		if ((ifs && strchr(ifs, words[*offset])) || (!ifs && strchr(" \t\n", words[*offset])))
			/* Reached IFS */
			break;
		...
		if (quoted != 1 && words[*offset] == '$') {
			error =
				parse_dollars(word, word_length, max_length, words, offset, flags, &glob_list, ifs,
							  ifs_white, quoted == 2);
			if (error)
				goto tidy_up;
			...
			if (quoted)
				...
				error = parse_backslash(word, word_length, max_length, words, offset);
	...
	*word = w_newword(word_length, max_length);
	for (i = 0; error == 0 && i < glob_list.we_wordc; i++)
		error =
			do_parse_glob(glob_list.we_wordv[i], word, word_length, max_length, pwordexp, ifs,
						  ifs_white);
...
static int
 parse_squote(char **word, size_t * word_length, size_t * max_length, const char *words,
			  size_t * offset)
{
...
char *my_itoa(long long int convertme, char* buffer, int len) {
	snprintf(buffer, len, "%lld", convertme);
	return buffer;
}

static int
 parse_arith(char **word, size_t * word_length, size_t * max_length, const char *words,
			 size_t * offset, int flags, int bracket)
{
	...
	for (; words[*offset]; ++(*offset)) {
		...
		case '$':
			error =
				parse_dollars(&expr, &expr_length, &expr_maxlen, words, offset, flags, NULL, NULL,
							  NULL, 1);
		...
		case '`':
			(*offset)++;
			error =
				parse_backtick(&expr, &expr_length, &expr_maxlen, words, offset, flags, NULL, NULL,
							   NULL);
		...
		case ')':
			if (--paren_depth == 0) {
				...
				result[20] = '\0';
				*word =
					w_addstr(*word, word_length, max_length, my_itoa(convertme,result,20));
				free(expr);
				return *word ? 0 : WRDE_NOSPACE;
			}
		...
		case ']':
			if (bracket && paren_depth == 1) {
				...
				result[20] = '\0';
				*word =
					w_addstr(*word, word_length, max_length,
							 my_itoa(numresult, result, 20));
				free(expr);
				return *word ? 0 : WRDE_NOSPACE;
			}
...
static int
 exec_comm(char *comm, char **word, size_t * word_length, size_t * max_length, int flags,
		   wordexp_t * pwordexp, const char *ifs, const char *ifs_white)
{
	int fildes[2];
	int bufsize = 128;
	...
	if (pid == 0) {
		...
		close(fildes[0]);
		execve(_PATH_BSHELL, (char *const *) args, environ);
	...
	if (!pwordexp) {			/* Quoted - no field splitting */
...
static int
 parse_comm(char **word, size_t * word_length, size_t * max_length, const char *words,
			size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
			const char *ifs_white)
{
	...
	for (; words[*offset]; ++(*offset)) {
		...
		case ')':
			if (!quoted && --paren_depth == 0) {
				...
				if (comm) {
					error =
						exec_comm(comm, word, word_length, max_length, flags, pwordexp, ifs,
								  ifs_white);
					free(comm);
				}
...
static int
 parse_param(char **word, size_t * word_length, size_t * max_length, const char *words,
			 size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
			 const char *ifs_white, int quoted)
{
	/* We are poised just after "$" */
  ...
  envsubst:
	...
	if (env == NULL) {
		if (seen_hash) {
			...
			buffer[20] = '\0';
			value = my_itoa(libc_argc - 1, buffer, 20);
			seen_hash = 0;
		} else {
			/* Just $ on its own */
			*offset = start - 1;
	...
	/* Is it a special parameter? */
		...
		if (*env == '$') {
			buffer[20] = '\0';
			value = my_itoa(getpid(), buffer, 20);
		}
		/* Is it `${#*}' or `${#@}'? */
		else if ((*env == '*' || *env == '@') && seen_hash) {
			buffer[20] = '\0';
			value = my_itoa(libc_argc > 0 ? libc_argc - 1 : 0, buffer, 20);
		...
		/* Is it `$*' or `$@' (unquoted) ? */
			...
			*end = 0;
			...
			if (libc_argc == 2)
				...
				value = w_addstr(*word, word_length, max_length, libc_argv[1]);
				if (value == NULL || w_addword(pwordexp, value))
					goto no_space;

				for (p = 2; libc_argv[p + 1]; p++) {
					char *newword = strdup(libc_argv[p]);
		...
		if (expand_pattern) {
			...
			for (p = pattern; p && *p; p++) {
				...
				case '$':
					offset = 0;
					error =
						parse_dollars(&expanded, &exp_len, &exp_maxl, p, &offset, flags, NULL, NULL,
									  NULL, 1);
					if (error) {
						if (free_value)
							free(value);
...
static int
 parse_dollars(char **word, size_t * word_length, size_t * max_length, const char *words,
			   size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
			   const char *ifs_white, int quoted)
{
	/* We are poised _at_ "$" */
	...
	default:
		...
		return parse_param(word, word_length, max_length, words, offset, flags, pwordexp, ifs,
						   ifs_white, quoted);
	}
}

static int
 parse_backtick(char **word, size_t * word_length, size_t * max_length, const char *words,
				size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
				const char *ifs_white)
{
	/* We are poised just after "`" */
...
static int
 parse_dquote(char **word, size_t * word_length, size_t * max_length, const char *words,
			  size_t * offset, int flags, wordexp_t * pwordexp, const char *ifs,
			  const char *ifs_white)
{
	...
	for (; words[*offset]; ++(*offset)) {
		...
		case '$':
			error =
				parse_dollars(word, word_length, max_length, words, offset, flags, pwordexp, ifs,
							  ifs_white, 1);
			/* The ``1'' here is to tell parse_dollars not to
		...
		case '`':
			...
			++(*offset);
			error =
				parse_backtick(word, word_length, max_length, words, offset, flags, NULL, NULL,
							   NULL);
			/* The first NULL here is to tell parse_backtick not to
			 * split the fields.
			 */
...
int wordexp(const char *words, wordexp_t * pwordexp, int flags)
{
	...
	for (words_offset = 0; words[words_offset]; ++words_offset)
		...
		case '$':
			error =
				parse_dollars(&word, &word_length, &max_length, words, &words_offset, flags,
							  pwordexp, ifs, ifs_white, 0);
		...
		case '`':
			...
			++words_offset;
			error =
				parse_backtick(&word, &word_length, &max_length, words, &words_offset, flags,
		...
		case '"':
			++words_offset;
			error =
				parse_dquote(&word, &word_length, &max_length, words, &words_offset, flags,
							 pwordexp, ifs, ifs_white);
		...
		case '~':
			error =
				parse_tilde(&word, &word_length, &max_length, words, &words_offset,
							pwordexp->we_wordc);
		...
		case '?':
			error =
				parse_glob(&word, &word_length, &max_length, words, &words_offset, flags, pwordexp,
						   ifs, ifs_white);
